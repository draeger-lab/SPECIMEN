...
def extract_cds(
    file: str, name: str, dir: str, collect_info: list, identifier: str
) -> str:
    ...
    match extension:
        ...
        case ".gbff":
            ...
            if os.path.isfile(Path(dir, "FASTA", name + "_nucs.fa")) and os.path.isfile(
                Path(dir, "FASTA", name + "_prot.fa")
            ):
                        ...
                        if record.features:
                            for feature in record.features:
                                ...
                                if feature.type == "CDS":
                                    ...
                                    # ---------------------------------
                                    temp_dict = {
                                        collect_info[0]: feature.qualifiers[
                                            collect_info[0]
                                        ][0]
                                    }
        ...
        case ".faa": # @TODO some bits still too rigid + better mapping? to allow more input handling
            ...
            info_df = pd.DataFrame(
                [seq.id for seq in SeqIO.parse(file, "fasta")], columns=["locus_tag"]
            )
            info_df.to_csv(Path(dir, name + "_info.csv"), index=False)
            fasta_name = file
            return fasta_name
...
def extract_bestbdbp_hits(tvq: str, qvt: str, name: str, cov: float = 0.25):
    ...
    logger.info(
        f'{len(out[out["reciprocal"]==1])} reciprocal and {len(out[out["reciprocal"]==0])} query blast best hits'
    )
